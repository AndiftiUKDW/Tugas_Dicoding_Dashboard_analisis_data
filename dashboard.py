...
def create_rfm(df):
    ...
    end = rfm_df['order_approved_at'].max()
    rfm_df["recentcy"] = rfm_df['order_approved_at'].apply(lambda x: (end-x).days)
    rfm_df = rfm_df.sort_values(by="recentcy",ascending=True)
...
filtered = rfm_df.sort_values(by="recentcy", ascending=True).head(5)
labelnya = [x[:3]+'...'+x[-2:] for x in filtered['customer_id']]
sns.barplot(y="recentcy", x="customer_id", data=rfm_df.sort_values(by="recentcy", ascending=True).head(5), palette=colors, ax=ax[0])
ax[0].set_ylabel(None)
ax[0].set_xlabel("customer", fontsize=30)
ax[0].set_title("By Recency (days)", loc="center", fontsize=50)
ax[0].tick_params(axis ='x', labelsize=25)
ax[0].tick_params(axis='y', labelsize=30)
ax[0].set_xticklabels(labelnya, rotation=45)
filtered = rfm_df.sort_values(by="frequency", ascending=False).head(5)
labelnya = [x[:3]+'...'+x[-2:] for x in filtered['customer_id']]
...
ax[1].set_xticklabels(labelnya, rotation=45)
filtered = rfm_df.sort_values(by="monetary", ascending=False).head(5)
labelnya = [x[:3]+'...'+x[-2:] for x in filtered['customer_id']]
...
ax[2].set_xticklabels(labelnya, rotation=45)
st.pyplot(fig)
